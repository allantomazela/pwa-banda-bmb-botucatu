...
import {
  ...
  type ContactInquiry,
} from '@/services/contact-inquiries'
import {
  Table,
  TableBody,
  TableCell,
...
export function InquiriesManager() {
  ...
  const fetchItems = async () => {
    setLoading(true)
    try {
      setItems(await getContactInquiries())
    } catch {
      setItems([])
    }
    setLoading(false)
  ...
  const handleStatus = async (id: string, status: string) => {
    const { error } = await updateContactInquiryStatus(id, status)
    if (error) {
  ...
  const handleDelete = async (id: string) => {
    const { error } = await deleteContactInquiry(id)
    if (error) {
      toast({ title: 'Erro', description: error, variant: 'destructive' })
      return
    }
