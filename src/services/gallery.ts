...
const SITE_IMAGE_TYPES: Record<'logo' | 'hero' | 'sponsor' | 'event', string[]> = {
  ...
  event: ['image/webp', 'image/jpeg', 'image/png'],
}
...
export async function uploadSiteImage(
  ...
  if (!allowed.includes(file.type)) {
    const formats =
      kind === 'hero' || kind === 'event' ? 'WebP, JPEG ou PNG' : 'WebP, PNG, SVG ou JPEG'
    return { url: null, error: `Formato inválido. Use ${formats}.` }
  }
  const maxBytes = kind === 'hero' || kind === 'event' ? 5 * 1024 * 1024 : 2 * 1024 * 1024
  if (file.size > maxBytes) {
    return {
      url: null,
      error:
        kind === 'hero' || kind === 'event'
