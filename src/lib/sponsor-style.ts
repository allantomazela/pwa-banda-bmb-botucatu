...
export function sponsorTierMeta(tier?: string | null) {
  const value = normalizeSponsorTier(tier)
  return SPONSOR_TIERS.find((item) => item.value === value) ?? SPONSOR_TIERS[1]
}

export function sponsorTierRank(tier?: string | null): number {
  return sponsorTierMeta(tier).rank
}
